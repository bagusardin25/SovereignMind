  ...
  return (
    <div className="max-w-4xl">
      ...
      <ol className="list-decimal list-inside space-y-2 mb-6 text-slate-300 text-sm pl-2">
        ...
        <li>Select <strong>Add Network</strong>, then choose <strong>Add a network manually</strong>.</li>
        ...
        <table className="w-full text-left border-collapse text-xs sm:text-sm">
          ...
          <tbody className="divide-y divide-slate-800/60">
            <tr className="hover:bg-slate-900/10">
              ...
              <td className="p-4 text-slate-300">Somnia Testnet</td>
              <td className="p-4 text-center">
                <button
                  onClick={() => copyToClipboard('Somnia Testnet', 'name')}
                  className="p-1 hover:text-white text-slate-500 rounded transition-colors"
                >
                  {copiedText === 'name' ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
                </button>
              </td>
            </tr>
            <tr className="hover:bg-slate-900/10">
              <td className="p-4 font-medium text-white">New RPC URL</td>
              <td className="p-4 text-slate-300 font-mono">https://dream-rpc.somnia.network</td>
              <td className="p-4 text-center">
                <button
                  onClick={() => copyToClipboard('https://dream-rpc.somnia.network', 'rpc')}
                  className="p-1 hover:text-white text-slate-500 rounded transition-colors"
                >
                  {copiedText === 'rpc' ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
                </button>
              </td>
            </tr>
            <tr className="hover:bg-slate-900/10">
              <td className="p-4 font-medium text-white">Chain ID</td>
              <td className="p-4 text-slate-300 font-mono">50312</td>
              <td className="p-4 text-center">
                <button
                  onClick={() => copyToClipboard('50312', 'chain')}
                  className="p-1 hover:text-white text-slate-500 rounded transition-colors"
                >
                  {copiedText === 'chain' ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
                </button>
              </td>
            </tr>
            <tr className="hover:bg-slate-900/10">
              <td className="p-4 font-medium text-white">Currency Symbol</td>
              <td className="p-4 text-slate-300 font-mono">STT</td>
              <td className="p-4 text-center">
                <button
                  onClick={() => copyToClipboard('STT', 'symbol')}
                  className="p-1 hover:text-white text-slate-500 rounded transition-colors"
                >
                  {copiedText === 'symbol' ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
                </button>
              </td>
            </tr>
            <tr className="hover:bg-slate-900/10">
              <td className="p-4 font-medium text-white">Block Explorer URL</td>
              <td className="p-4 text-slate-300 font-mono">https://shannon.somnia.network</td>
              <td className="p-4 text-center">
                <button
                  onClick={() => copyToClipboard('https://shannon.somnia.network', 'explorer')}
                  className="p-1 hover:text-white text-slate-500 rounded transition-colors"
                >
                  {copiedText === 'explorer' ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
                </button>
              </td>
            </tr>
          </tbody>
      ...
      {/* Faucet Callout */}
      <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 mb-6 flex gap-4">
        <Info className="text-purple-400 shrink-0 mt-1" size={20} />
        <div>
          <h4 className="font-semibold text-slate-200 text-sm mb-1">Acquiring STT Test Tokens</h4>
          ...
          <a
            ...
            className="inline-flex items-center gap-1.5 bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 border border-purple-500/30 font-semibold text-xs px-4 py-2 rounded-lg transition-all"
          >
      ...
      <p className="text-slate-300 mb-6 leading-relaxed text-sm sm:text-base">
        Once your wallet is configured and funded with testnet STT, go to the SovereignMind dashboard and click the <strong>Connect Wallet</strong> button in the top right header. Once connected, your wallet will automatically sync, showing your STT balance and portfolio shares.
      </p>
